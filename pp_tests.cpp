...
TEST_F(test_fixture, only_parse) {
    ASSERT_EQ(parse_expression(&uut, &symtab), UNDECLARED_IDENTIFIER);
}
...
TEST_F(lexical_error, only_parse) {
    ASSERT_EQ(parse_expression(&uut, &symtab), LEXICAL_ERROR);
}
...
TEST_F(basic, only_parse) {
    ASSERT_EQ(parse_expression(&uut, &symtab), EXPRESSION_SUCCESS);
}
...
TEST_F(relational_operators3, only_parse) {
    ASSERT_EQ(parse_expression(&uut, &symtab), SEM_ERROR_IN_EXPR);
}
...
TEST_F(parenthesis_err, only_parse) {
    ASSERT_EQ(parse_expression(&uut, &symtab), EXPRESSION_SUCCESS); /**< PP parser should return control to the topdown parser*/
}
...
TEST_F(op_err1, only_parse) {
    ASSERT_EQ(parse_expression(&uut, &symtab), EXPRESSION_FAILURE);
}
...
TEST_F(undeclared3, only_parse) {
    ASSERT_EQ(parse_expression(&uut, &symtab), UNDECLARED_IDENTIFIER); /**< PP parser should return control to the topdown parser*/
}
